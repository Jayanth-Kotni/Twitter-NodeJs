...
const convertUserObjToResponse = dbObject => {
  return {
...
const convertFollowerToResponseObj = dbObject => {
  return {
...
const convertTweetToResponseDbObj = dbObject => {
  return {
...
const convertReplyToResponseDbObj = dbObject => {
  return {
...
const convertLikeToResponseObj = dbObject => {
  return {
...
const authenticateUser = (request, response, next) => {
  let jwtToken
  const authorizeHeader = request.headers['authorization']
  if (!authorizeHeader) {
    response.status(401)
    response.send('Invalid JWT Token')
  } else {
    jwtToken = authorizeHeader.split(' ')[1]
...
app.get('/user/tweets/feed/', authenticateUser, async (request, response) => {
  ...
  `
  const followingUserObjList = await database.all(followingQuery)
  const followingList = followingUserObjList.map(object => {
...
app.get('/user/following/', authenticateUser, async (request, response) => {
  ...
  const followingQuery = `
    ...
    WHERE follower_user_id = ${dbUser.user_id};
  `
  const followingUsersObjList = await database.all(followingQuery)
  const followingList = followingUsersObjList.map(object => {
    return object['following_user_id']
  })
